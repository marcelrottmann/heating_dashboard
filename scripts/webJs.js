...
var reportDataSetToday = []
var reportDataSetTodayLabels = getAll15Minutes()
var labels = []
var tempLabels = []
...
function showReportData(x) {
    ...
    if (x == "myChart6") { data.datasets = reportDataSetToday; data.labels = reportDataSetTodayLabels }
    ...
    if (x == "myChart3" || x == "myChart4" || x == "myChart5" || x == "myChart6") { config.options = { maintainAspectRatio: false } }
    if (x == "myChart6") {
        // /config.type = "bar"
...
function refreshHeatingVisuals(data) {
    ...
    zonesArray.forEach(function (x, index) {
        ...
        if (x.dataSection && x.type != "HOT_WATER") {
            ...
            var tempData = {
                "labels": getAll15Minutes(),
                dataset: {
                    "label": x.name,
                    "backgroundColor": chartColours[index + 10],
                    "borderColor": chartColours[index + 10],
                    "data": x.histSection.heatingIntervals
                }
            }

            reportDataSetToday.push(tempData.dataset)
    ...
    //Branding Bar
    innerContent = document.createElement('canvas')
    innerContent.setAttribute("id", "myChart6")
    innerContent.setAttribute("style", "padding:5%")
    tempDiv.appendChild(innerContent)
    tilesCollection.push(tempDiv)
...
function getAll15Minutes() {
    var tDate = new Date()
    var x = new Date(tDate.toISOString().substring(0, 16))
    var y = new Date(x.toISOString().split("T")[0])
    console.log(x,y)
    array = []
    i = 0
    while (i < 96) {
        if (i !== 0) { y.setSeconds(y.getSeconds() + (60*15)) }
        array.push(y.toISOString().substring(0, 16))
        i++
    }
    return array
}